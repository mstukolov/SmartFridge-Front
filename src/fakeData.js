...
function randomCommercialNetwork() {
  return chains[Object.keys(chains)[randomInteger(0, 3)]];
}
...
for (let i = 0; i < 20; i++) {
  ...
  if (i < 5) chains["Aschan_ID"].storess.push(pointId);
  if (i > 4 && i <= 9) chains["OK_ID"].storess.push(pointId);
  if (i > 9 && i <= 14) chains["5_ID"].storess.push(pointId);
  if (i > 14 && i <= 19) chains["Perekrestok_ID"].storess.push(pointId);

  stores[pointId] = point;
}
...
export const equipment = {
  "5499b724-c331-4f29-b25f-55583c0ece34": {
    ...
    stores: "",
    chains: randomCommercialNetwork().id
  },
...
for (let key in equipment) {
  const network = equipment[key].chains;
  let stores = equipment[key].stores;
  switch (network) {
    case "Aschan_ID":
      stores = `point_${randomInteger(0, 4)}_ID`;
      break;

    case "OK_ID":
      stores = `point_${randomInteger(5, 9)}_ID`;
      break;

    case "5_ID":
      stores = `point_${randomInteger(10, 14)}_ID`;
      break;

    case "Perekrestok_ID":
      stores = `point_${randomInteger(15, 19)}_ID`;
      break;
